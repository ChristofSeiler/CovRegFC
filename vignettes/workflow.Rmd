...
condition = "task"
# select session
data = subset(data,session == 2)
# subsample time points within each participant
n_tp = 4000
subsample_ids = lapply(levels(data$participant),function(part_id) {
  ids = which(data$participant == part_id)
  sample(x = ids,size = round(n_tp/length(levels(data$participant))))
}) %>% unlist
data_subset = data[subsample_ids,] %>% droplevels
# subsample subjects
#data_subset = data[data$participant %in% 1:2,] %>%
#  droplevels
