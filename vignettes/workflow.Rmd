...
region_list = list(frontal=frontal,
...
```{r}
region_location = lapply(1:length(region_list),function(i) {
  region = region_list[[i]]
  coord = channel_location[str_detect(region,
                              channel_location$names),
                   c("x","y")] %>% colMeans
  data.frame(names=names(region_list)[i],x = coord[1],y = coord[2])
}) %>% do.call(rbind,.)
rownames(region_location) = NULL
region_location$names = as.character(region_location$names)
plot_channels(region_location,
              x_label = "left <--> right",
              y_label = "posterior <--> anterior")
```
...
condition = "task"
# select session
data_subset = subset(data,session == 2) %>% .[.$participant %in% 1:2,] %>% droplevels
# subsample time points within each participant
# n_tp = 10000
# subsample_ids = lapply(levels(data_subset$participant),function(part_id) {
#   ids = which(data_subset$participant == part_id)
#   sample(x = ids,size = round(n_tp/length(levels(data_subset$participant))))
# }) %>% unlist
# data_subset = data_subset[subsample_ids,]
fit = covreg(data_subset,response,condition)
```

Traceplots to assess convergence of HMC sampler.

```{r}
plot_unexplained_cov_diagnostics(fit)
plot_explained_cov_diagnostics(fit,num_par = 4)
```

Analyze posterior covariance **unexplained** by simulation conditions.

```{r fig.height=4,fig.width=7}
plot_unexplained_cov(fit,response)
```

Analyze posterior covariance **explained** by simulation conditions.

```{r}
plot_explained_cov(fit,data_subset,response,condition)
plot_explained_cov_channels(fit,data_subset,response,condition,
                            region_location,
                            x_label = "left <--> right",
                            y_label = "posterior <--> anterior")
```
